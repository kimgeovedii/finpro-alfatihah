...
import { Branch, BranchSchedule, DayName } from "../types/branch-admin.type";
...
export const BranchScheduleDialog: React.FC<BranchScheduleDialogProps> = ({
  ...
  const hasUnsetDays = schedules.some(s => s.isPlaceholder);
  
  // Available days to select: unset days, plus the currently selected day if we are editing it
  const availableDays = DAYS.filter(day => 
    schedules.find(s => s.dayName === day)?.isPlaceholder || (isEditing && day === formik.values.dayName)
  );
  ...
  React.useEffect(() => {
    if (!isEditing && availableDays.length > 0 && !availableDays.includes(formik.values.dayName)) {
      formik.setFieldValue("dayName", availableDays[0]);
    }
  }, [availableDays, formik, isEditing]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-6 py-4 max-h-[70vh] overflow-y-auto pr-1">
          ...
          {(hasUnsetDays || isEditing) && (
            <div className="p-4 border border-emerald-100 bg-emerald-50/30 rounded-xl space-y-4 relative">
              {isEditing && (
                <Button 
                  variant="ghost" 
                  size="sm" 
                  className="absolute top-2 right-2 h-7 px-2 text-[10px] text-slate-400 hover:text-slate-600"
                  onClick={() => {
                    setIsEditing(false);
                    formik.resetForm();
                  }}
              ...
              <h4 className="text-sm font-semibold text-emerald-800">
                {isEditing ? "Update" : "Add"} Operating Hours
              </h4>
