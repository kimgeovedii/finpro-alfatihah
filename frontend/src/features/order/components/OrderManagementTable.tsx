...
export const OrderManagementTable: React.FC<Props> = ({
  orders,
  meta,
  isLoading,
  activeStatus,
  onPageChange,
  onValidatePaymentEvidence,
  role,
}) => {
  // Pagination
              ...
              <TableRow>
                <TableCell colSpan={7} className="text-slate-400 py-10">
                  <MessageBox
                    ...
                    description={`No <b>${activeStatus}</b> order / transaction found`}
                  />
                </TableCell>
              </TableRow>
            ) : (
                ...
                return (
                  <TableRow key={dt.id} className="hover:bg-slate-50">
                    ...
                    <TableCell className="text-slate-600">
                      {formatDate(dt.createdAt, true)}
                    </TableCell>
                    <TableCell className="text-slate-800 font-medium">
                      Rp {dt.finalPrice.toLocaleString(currencyFormat)}
                    </TableCell>
                    <TableCell className="max-w-[140px]">
                      <Badge
                        className={`capitalize px-2 font-semibold whitespace-normal wrap-break-word text-center h-auto mx-auto block rounded-lg ${statusClass}`}
                      >
                        {finalStatus}
                      </Badge>
                    </TableCell>
                    {role === "SUPER_ADMIN" && (
                      <TableCell>
                        ...
                        dt.payments.length > 0 &&
                        dt.payments[0].method === "MANUAL" &&
                        dt.payments[0].evidence !== null ? (
                          <PaymentConfirmationDialog
                            imageUrl={dt.payments[0].evidence ?? ""}
                            finalPrice={dt.finalPrice}
                            paymentId={dt.payments[0].id}
                            status={dt.status}
                            onValidatePaymentEvidence={
                              onValidatePaymentEvidence
                            }
