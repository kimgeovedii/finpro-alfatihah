...
export const useProductActions = (product: ProductDetailData | null, fetchProduct: () => void) => {
  ...
  const handleCreateCart = useCallback(async (branchId: string, productId: string, qty: number) => {
  ...
  const handleRemoveCartItem = useCallback(async (cartItemId: string, productName: string) => {
  ...
  const cartProps = {
    ...
    totalPrice: (product?.basePrice || 0) * qty,
    isCreating,
    onAddToCart: () =>
      isAvailable && product &&
