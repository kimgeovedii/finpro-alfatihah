...
export const AddressSelectionCard: React.FC<Props> = ({ branch, address }) => {
    return (
        <div className="bg-white rounded-3xl">
                ...
                <div className="absolute inset-0 opacity-10 pointer-events-none" style={{ backgroundImage: "repeating-linear-gradient(45deg, #fff 0, #fff 1px, transparent 0, transparent 50%)", backgroundSize: "12px 12px" }}/>
                    <div className="relative flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="inline-block text-[11px] font-semibold text-slate-500 bg-green-600 px-2 py-0.5 rounded-md">
            ...
            <div className="flex justify-between items-center my-3 px-3">
                ...
                <button className="text-emerald-700 text-sm font-bold hover:text-emerald-600 transition-colors">Change</button>
            </div>
            <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm m-4">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        <p className="text-slate-800 font-bold text-sm mb-0.5">{address.label}</p>
                        <p className="text-slate-500 text-sm leading-snug mb-1.5">{address.address}</p>
                        <div className="flex items-center gap-2 text-slate-500 text-sm">
                            <span className="mt-1 text-xs font-semibold text-slate-500 bg-slate-100 px-2 py-1 rounded-md flex gap-1">
                                <UserIcon className="w-4 h-4"/> {address.recipientName}
                            </span>
                            <span className="mt-1 text-xs font-semibold text-slate-500 bg-slate-100 px-2 py-1 rounded-md flex gap-1">
                                <PhoneIcon className="w-4 h-4"/> {address.phone}
                            </span>
                            <span className="mt-1 text-xs font-semibold text-slate-500 bg-slate-100 px-2 py-1 rounded-md flex gap-1">
                                <MapIcon className="w-4 h-4"/> {address.distance} Km
                            </span>
                        </div>
                    </div>
                </div>
