...
export const ProductOrderCartItemCard: React.FC<ProductOrderCartCardProps> = ({ item, variant = "order", onIncrease, onDecrease, onRemove }) => {
    return (
        <div className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
            <div className="w-14 h-14 rounded-xl bg-slate-100 flex items-center justify-center flex-shrink-0 overflow-hidden">
                {item.imageUrl ? <img src={item.imageUrl} alt={item.productName} className="w-full h-full object-cover"/> : <span className="text-2xl">🛒</span>}
            </div>
            <div className="flex-1 min-w-0">
                <p className="text-[10px] font-semibold text-emerald-600 uppercase tracking-wider">{item.category}</p>
                <p className={`text-sm font-semibold text-slate-800 ${variant === "order" ? "line-clamp-3" : ""}`}>{item.productName}</p>
