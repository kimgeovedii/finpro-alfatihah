...
export const MessageBox: React.FC<Props> = ({ context, description, image, urlButton, titleButton } ) => {
    return (
        <div className="p-5 rounded-lg bg-gray-100 border-1 border-gray-200 mx-auto text-center">
            ...
            <p className="text-teal-700 font-medium text-md">- {context} -</p>
            { description && <p className="text-gray-500 text-sm" dangerouslySetInnerHTML={{ __html: description }}></p> }
            { 
                urlButton && titleButton && 
                    <Link href={urlButton}>
                        <Button className="bg-teal-700 text-sm p-4 mt-4"><ArrowRightIcon/> {titleButton}</Button> 
                    </Link>
            }
        </div>
    )
