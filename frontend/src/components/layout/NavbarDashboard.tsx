...
export const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 flex h-20 md:h-20 w-full items-center justify-between bg-white px-4 md:px-8 shadow-sm">
      {/* Brand Logo & Name */}
      ...
      <div className="flex items-center gap-4 md:gap-6 justify-end w-auto">
        ...
        <div className="flex items-center gap-3 md:gap-6 border-l border-slate-100 pl-4 md:pl-8">
          <div className="text-right hidden sm:block">
            <p className="text-sm font-black text-[#0a2e2e] leading-tight truncate max-w-[120px] lg:max-w-none">
              {user?.username || "Admin"}
            </p>
          ...
          <div className="h-8 w-8 md:h-12 md:w-12 rounded-full border-2 border-emerald-500 p-0.5 shadow-sm shrink-0">
            <img
              src={
                user?.avatar ||
                `https://ui-avatars.com/api/?name=${user?.username || "Admin"}&background=10b981&color=fff&bold=true`
              }
              alt="Profile"
